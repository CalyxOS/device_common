...
then
cat >> tmp/$PRODUCT-$VERSION/flash-all.sh << EOF
fastboot flash --slot=other radio_a radio-$DEVICE-$RADIO.img || exit $?
fastboot --set-active=other reboot-bootloader || exit $?
sleep $SLEEPDURATION
fastboot flash --slot=other radio_b radio-$DEVICE-$RADIO.img || exit $?
fastboot --set-active=other reboot-bootloader || exit $?
sleep $SLEEPDURATION
EOF
fi
...
then
cat >> tmp/$PRODUCT-$VERSION/flash-all.bat << EOF
fastboot flash --slot=other bootloader_a bootloader-$DEVICE-$BOOTLOADER.img || exit 1
fastboot --set-active=other reboot-bootloader || exit 1
ping -n $SLEEPDURATION 127.0.0.1 >nul
fastboot flash --slot=other bootloader_b bootloader-$DEVICE-$BOOTLOADER.img || exit 1
fastboot --set-active=other reboot-bootloader || exit 1
ping -n $SLEEPDURATION 127.0.0.1 >nul
EOF
fi
...
then
cat >> tmp/$PRODUCT-$VERSION/flash-all.bat << EOF
fastboot flash --slot=other radio_a radio-$DEVICE-$RADIO.img || exit 1
fastboot --set-active=other reboot-bootloader || exit 1
ping -n $SLEEPDURATION 127.0.0.1 >nul
fastboot flash --slot=other radio_b radio-$DEVICE-$RADIO.img || exit 1
fastboot --set-active=other reboot-bootloader || exit 1
ping -n $SLEEPDURATION 127.0.0.1 >nul
EOF
fi
...
then
cat >> tmp/$PRODUCT-$VERSION/flash-base.sh << EOF
fastboot flash --slot=other bootloader_a bootloader-$DEVICE-$BOOTLOADER.img || exit $?
fastboot --set-active=other reboot-bootloader || exit $?
sleep $SLEEPDURATION
fastboot flash --slot=other bootloader_b bootloader-$DEVICE-$BOOTLOADER.img || exit $?
fastboot --set-active=other reboot-bootloader || exit $?
sleep $SLEEPDURATION
EOF
fi
