...
fastboot getvar product | grep "^product: $PRODUCT$"
if [ $? -ne 0 ]; then
...
fastboot getvar product | findstr /r /c:"^product: $PRODUCT" || echo "Factory image and device do not match. Please double check"
fastboot getvar product | findstr /r /c:"^product: $PRODUCT" || exit /B 1
EOF
if test "$UNLOCKBOOTLOADER" = "true"
then
cat >> tmp/$PRODUCT-$VERSION/flash-all.bat << EOF
